...
- 2026-09-24 09:10 合并 main 后的 DMG 重新交付：`cargo build --release --locked --target aarch64-apple-darwin`（5m04s）与 updater 构建通过；`bash .github/scripts/bundle-macos.sh aarch64-apple-darwin arm64` 生成 `dist/tty7-26.9.3-macos-arm64.dmg`（28217178 bytes，SHA-256 `4aa945bc2e277720ac980b1d42dceb98e16b6377c9a0c1c9af31dd0564dcced6`）与 `dist/tty7-26.9.3-macos-arm64.zip`（24117153 bytes，SHA-256 `3a63db436815cbafa146c1697088507651da10439175d657c26bdc19be118d3f`）。DMG CRC VALID，只读挂载含 `tty7.app` 与 `/Applications` 入口，`CFBundleShortVersionString=26.9.3`，包内 `tty7-app`（`9EA82965-F761-388E-A1A0-32DD3C1DFCEE`）、`tty7`（`6E3474E9-D444-3F6F-991F-E1A9E5A74900`）、`tty7-updater`（`EB75E05E-58C7-3A15-AD6C-C5B89B4D64AC`）均为 thin arm64 且 UUID 与 release 产物逐一比对一致，app 签名验证通过。打包前旧 ZIP 已备份至 `/private/tmp/tty7-dist-backup-repack-20260924`。打包前验证：`cargo fmt --check`、`git diff --check`、cli_agent 38、agent_hooks 52、ui::settings 62、terminal::remote 94 项测试全部通过。
